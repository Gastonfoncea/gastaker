...
describe('HTTP: invites y register', () => {
  ...
  it('register exitoso: 201, cookie, 10 categorías, ingest_token, y auto-login', async () => {
    const inv = await newInvite()
    ...
    // Seed de 10 categorías + ingest_token.
    const me = (await agent.get('/api/me')).body
    expect(me.ingest_token).toMatch(/^[0-9a-f]{48}$/)
    expect((await agent.get('/api/categories')).body.categories).toHaveLength(10)
  })
